...
class ColorPicker(QtGui.QWidget):
    ...
    def onCurrentColorChanged(color):
        ...
        data = [255 - i for i in ColorPicker.rgb_color_array]
        data_as_str = ', '.join(str(i) for i in
                                data)  # we join the array elements with a comma and a space since we need to send a string through the serial

        global lasttime
        if time.time() - lasttime >= interval:  # we send data only between a certain interval because arduino cannot process it that quick
            print(data_as_str)
            arduinoBoardSerial.write(
                (str(data_as_str)).encode())  # when sending data make sure to send an encoded string
            lasttime = time.time()
...
def run():
    ...
    PORT = "COM5"  # define device port (make sure you close serial monitor on Arduino IDE
    ...
    # !Important, changing baud rate might affect communication speed by a lot, try what fits best for your use
